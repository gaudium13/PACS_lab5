...
int main(int argc, char** argv)
{
  ...
  kernel = clCreateKernel(program, "flip_img", NULL);
  cl_error(err, "Failed to create kernel from the program.\n");

  printf("Host info declared");
  cl_mem in_red_channel = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned char)*himg_width*himg_height, NULL, &err);
  cl_mem in_green_channel = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned char)*himg_width*himg_height, NULL, &err);
  cl_mem in_blue_channel = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned char)*himg_width*himg_height, NULL, &err);
  ...
  err = clSetKernelArg(kernel, 3, sizeof(unsigned int), &himg_width);
  cl_error(err, "Failed to set argument 3\n");
  err = clSetKernelArg(kernel, 4, sizeof(unsigned int), &himg_height);
  ...
  err = clEnqueueReadBuffer(command_queue, in_red_channel, CL_TRUE, 0, sizeof(unsigned char)*himg_width*himg_height, img_red, 0, NULL, &timing_event_read);
  err = clEnqueueReadBuffer(command_queue, in_green_channel, CL_TRUE, 0, sizeof(unsigned char)*himg_width*himg_height, img_green, 0, NULL, &timing_event_read);
  err = clEnqueueReadBuffer(command_queue, in_blue_channel, CL_TRUE, 0, sizeof(unsigned char)*himg_width*himg_height, img_blue, 0, NULL, &timing_event_read);
  ...
  cimg_library::CImg<unsigned char> flipped_img(himg_height, himg_width, 1, 3);

  for (unsigned int row = 0; row < himg_height; row++)
  {
	for (unsigned int column = 0; column < himg_width; column++)
	{
		flipped_img(column, row, 0, 0) = img_red[row*himg_width+column];
		flipped_img(column, row, 0, 1) = img_green[row*himg_width+column];
		flipped_img(column, row, 0, 2) = img_blue[row*himg_width+column];
	}
  }
